...
// The data layer. Two independent feeds, joined in Model.js:
//
//   * one JSON record per agent under the usage directory, maintained by
//     `omarchy-agent-usage-update` -- the source of token history;
//   * `ai-usagebar usage --json` -- the source of pace detail, severities,
//     and every provider that has no collector of its own.
//
// Neither is required. A machine with no ai-usagebar still gets the panel it
// had before; a provider that only ai-usagebar knows about still gets a tab.
Item {
  id: root
  visible: false
  ...
  // Which providers ship an `assets/<id>-light.svg` twin for light surfaces.
  //
  // Marks still resolve by convention -- drop a file in `assets/` and it is
  // used -- but the convention is read from the directory once at startup
  // rather than by pointing an Image at every possible name and letting the
  // misses fail. Probing by failure works, but it logs "Cannot open" on every
  // panel open under a light theme for each provider whose mark needs no
  // twin, and a warning that is always wrong trains you to ignore warnings.
  property var lightMarkIds: ({})

  Process {
    id: markListProcess
    running: false
    command: ["find", Qt.resolvedUrl("assets").toString().replace(/^file:\/\//, ""),
              "-maxdepth", "1", "-name", "*-light.svg", "-printf", "%f\n"]
    ...
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          if (name.slice(-10) === "-light.svg") found[name.slice(0, -10)] = true
        }
        root.lightMarkIds = found
      }
    }
  }

  function hasLightMark(providerId) {
    return lightMarkIds[String(providerId)] === true
  }
  ...
  // full refresh interval. Only the advising agents rerun, so an outage at one
  // provider does not put every other collector on a 30-second treadmill.
  ...
  Process {
    ...
    stderr: StdioCollector {
      waitForEnd: true
      onStreamFinished: if (text.trim() !== "") console.warn("ai-agents", text.trim())
    }
  }
  ...
  // walk over every transcript on disk -- the collectors reuse their recent
  // scans in this mode.
  ...
  property var vendorEntries: []
  property string vendorError: ""
  property bool vendorAvailable: false
  property int vendorRevision: 0
  property bool vendorRefreshQueued: false

  Process {
    id: vendorProcess
    running: false
    // /usr/bin/env always starts on Omarchy and reports a missing ai-usagebar
    // as exit 127, which is how an uninstalled binary is told apart from one
    // that ran and failed.
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: vendorProcess.buffer = text
    }

    stderr: StdioCollector {
      waitForEnd: true
      onStreamFinished: if (text.trim() !== "") console.warn("ai-agents/usagebar", text.trim())
    }

    onExited: function(exitCode) {
      // Let the waitForEnd collector publish its buffer first.
      Qt.callLater(function() { root.finishVendorRefresh(exitCode) })
    }
  }

  function startVendorRefresh() {
    if (vendorProcess.running) {
      vendorRefreshQueued = true
      return
    }
    vendorProcess.buffer = ""
    vendorProcess.running = true
  }

  function finishVendorRefresh(exitCode) {
    if (exitCode === 127) {
      // Not installed. That is a supported configuration, not an error: the
      // collectors alone still fill the panel.
      ...
      vendorAvailable = false
      vendorEntries = []
      vendorError = "ai-usagebar exited " + exitCode
    } else {
      var feed = Model.parseVendorFeed(vendorProcess.buffer)
      vendorAvailable = feed.ok
      vendorEntries = feed.entries
      vendorError = feed.error
    }
    vendorRevision++

    if (vendorRefreshQueued) {
      vendorRefreshQueued = false
      Qt.callLater(root.startVendorRefresh)
    }
  }
  ...
  // settings and having actually produced numbers -- locally, on a synced
  // device, or through ai-usagebar. With nothing to show, the whole module
  // collapses out of the bar rather than sitting there dimmed.
  property var providers: {
    var rev = dataRevision
    var syncRev = syncRevision
    var vendorRev = vendorRevision
    ...
    var syncedProviders = syncConfigured() && aggregateData && aggregateData.providers
      ? aggregateData.providers : {}
    for (var syncedId in syncedProviders) {
      var alreadyLocal = false
      for (var r = 0; r < records.length; r++)
        if (String(records[r].id) === syncedId) { alreadyLocal = true; break }
      if (!alreadyLocal)
        records.push({ id: syncedId, name: syncedProviders[syncedId].providerName || syncedId })
    }

    return Model.buildProviders(records, historyFor, vendorEntries, providerEnabled)
  }

  // Where a provider's token history comes from: the synced aggregate when
  // cross-device merging is on and knows this provider, the local record
  // otherwise.
  function historyFor(agentId, record) {
    var stats = syncedStatsFor(agentId)
    if (stats) {
      return {
        todayPrompts: stats.todayPrompts, todaySessions: stats.todaySessions,
        todayTotalTokens: stats.todayTotalTokens,
        todayTokensByModel: stats.todayTokensByModel || ({}),
        recentDays: stats.recentDays || [], totalPrompts: stats.totalPrompts,
        totalSessions: stats.totalSessions, activeDays: stats.activeDays,
        modelUsage: stats.modelUsage || ({}),
        hasLocalStats: stats.hasLocalStats !== false,
        hasPromptStats: stats.hasPromptStats !== false,
        syncEnabled: true,
        syncDeviceCount: Number(stats.deviceCount || (aggregateData ? aggregateData.deviceCount : 0) || 0)
      }
    }
    if (!record) return null
    return {
      todayPrompts: record.todayPrompts, todaySessions: record.todaySessions,
      todayTotalTokens: record.todayTotalTokens,
      todayTokensByModel: record.todayTokensByModel || ({}),
      recentDays: record.recentDays || [], totalPrompts: record.totalPrompts,
      totalSessions: record.totalSessions, activeDays: record.activeDays,
      modelUsage: record.modelUsage || ({}),
      hasLocalStats: record.hasLocalStats !== false,
      hasPromptStats: record.hasPromptStats !== false,
      syncEnabled: false, syncDeviceCount: 0
    }
  }
  ...
  readonly property string syncSnapshotPath: syncConfigured() ? syncEffectiveDir + "/" + syncEffectiveFileName : home + "/.cache/omarchy/ai-agents-disabled.json"
  ...
  Process {
    ...
    stderr: StdioCollector {
      waitForEnd: true
      onStreamFinished: if (text.trim() !== "") console.warn("ai-agents/sync", text.trim())
    }
  }
  ...
  function parseSyncScanOutput(output) {
    ...
    function flush() {
      ...
      try {
        ...
        console.warn("ai-agents/sync", "Ignoring bad snapshot", currentPath, e)
      }
      currentPath = ""
      currentJson = []
    }
  ...
  // (Fireworks' billing API) are replicas of the same upstream truth on every
  // synced device, so the widest value wins -- summing them would double every
  // token per machine.
  function combineNumber(additive, current, value) {
    return additive ? numberValue(current) + numberValue(value)
                    : Math.max(numberValue(current), numberValue(value))
  }
  ...
  function aggregateSnapshots(snapshots) {
    ...
    function providerAcc(id) {
      ...
      providers[id] = {
        providerId: id, providerName: "", ready: false, hasLocalStats: false,
        hasPromptStats: false, todayPrompts: 0, todaySessions: 0,
        todayTotalTokens: 0, todayTokensByModel: ({}), recentByDay: recentByDay,
    ...
    for (var id in providers) {
      ...
      for (var di = 0; di < dates.length; di++)
        recentDays.push({ date: dates[di], messageCount: entry.recentByDay[dates[di]] || 0 })
      var providerDevices = Object.keys(entry.devices).sort()
      outProviders[id] = {
        providerId: entry.providerId, providerName: entry.providerName,
        ready: entry.ready || providerDevices.length > 0,
        hasLocalStats: entry.hasLocalStats, hasPromptStats: entry.hasPromptStats,
        todayPrompts: entry.todayPrompts, todaySessions: entry.todaySessions,
        todayTotalTokens: entry.todayTotalTokens,
        todayTokensByModel: entry.todayTokensByModel, recentDays: recentDays,
        totalPrompts: entry.totalPrompts, totalSessions: entry.totalSessions,
        activeDays: Math.max(entry.activeDays, Object.keys(entry.activeDates).length),
        modelUsage: entry.modelUsage, deviceCount: providerDevices.length,
        devices: providerDevices
      }
    }

    return {
      schemaVersion: 1, updatedAt: new Date().toISOString(), updatedAtMs: Date.now(),
      deviceCount: Object.keys(devices).length, devices: Object.keys(devices).sort(),
      providers: outProviders
    }
  }

  // Snapshots keep the field names Omarchy's own agents widget writes, so a
  // fleet of machines running either widget still merges cleanly both ways.
  function providerSnapshot(record) {
    return {
      providerId: String(record.id), providerName: String(record.name || record.id),
      ready: record.ready === true, hasLocalStats: record.hasLocalStats !== false,
      hasPromptStats: record.hasPromptStats !== false,
  ...
  function localSnapshot() {
    ...
    return {
      schemaVersion: 1, deviceId: syncEffectiveDeviceId,
      updatedAt: new Date().toISOString(), providers: providerMap
    }
  }
  ...
  function formatTokenCount(n) { return Model.formatTokenCount(n) }
  function friendlyModelName(id) { return Model.friendlyModelName(id) }
}
